  ...
  useEffect(() => {
    if (typeof slug === 'string') {
      ...
      if (fairData) {
        ...
        // Get artists data
        const artistsData = fairData.artists
          .map((artistId: string) => getArtistById(artistId))
          .filter(Boolean)
          ...
          <div className='container mx-auto px-4 pb-12'>
            ...
            >
              ...
              <p className='mt-4 text-xl'>
                {formatDate(fair.startDate, language)} -{' '}
                {formatDate(fair.endDate, language)}
              </p>
              <p className='mt-2 text-lg'>
                {language === 'es' ? fair.location.es : fair.location.en} |{' '}
                {fair.boothNumber}
              </p>
            </motion.div>
          </div>
      ...
      <div className='container mx-auto px-4 py-12'>
        ...
        >
          ...
          <div className='max-w-3xl'>
            <BlockContent
              blocks={
                language === 'es' ? fair.description.es : fair.description.en
              }
            />
          </div>
        </motion.div>
        ...
        {relatedNews.length > 0 && (
          ...
          >
            <RelatedNews
              news={relatedNews}
              title={
                language === 'es' ? 'Noticias sobre la feria' : 'Fair News'
              }
            />
          </motion.div>
        )}
